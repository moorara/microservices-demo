...
describe('GraphQL Server', function () {
  ...
  describe('GraphQL', () => {
    ...
    it('createSite', async () => {
      ...
      for (const { name, location, priority, tags } of inputs) {
    ...
    it('sensors', async () => {
      for (const { id: siteId } of store.sites) {
        const res = await query(`
          query {
    ...
    it('site', async () => {
      for (const { id, name, location, priority, tags } of store.sites) {
        const res = await query(`
          query {
            site (id: "${id}") {
              id
    ...
    it('sensor', async () => {
      for (const { id, siteId, name, unit, minSafe, maxSafe } of store.sensors) {
        const res = await query(`
          query {
            sensor (id: "${id}") {
              id
    ...
    it('switch', async () => {
      for (const { id, siteId, name, state, states } of store.switches) {
        const res = await query(`
          query {
            switch (id: "${id}") {
              id
    ...
    it('asset', async () => {
      for (const { id, siteId, serialNo, material } of store.alarms) {
        const res = await query(`
          query {
            asset (id: "${id}") {
              id
      ...
      for (const { id, siteId, serialNo, resolution } of store.cameras) {
        const res = await query(`
          query {
            asset (id: "${id}") {
              id
    ...
    it('removeSwitch', async () => {
      for (const { id } of store.switches) {
        const res = await query(`
          mutation {
            removeSwitch (id: "${id}")
          }
